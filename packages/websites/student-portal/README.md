...
Run `pnpm nx build @lms/student-portal [--skip-nx-cache]` to build the application.

## Run dev server

Run `pnpm nx serve @lms/student-portal`

## Running unit tests

Run `pnpm nx test @lms/student-portal` to execute the unit tests via Vitest.

### Updating snapshots

To update snapshots, run the following command:

`pnpm nx test @lms/student-portal --configuration=update-snapshot`

## Run lint

Run `pnpm nx lint @lms/student-portal`

### Fixable issues

You can also automatically fix some lint errors by running the following command:

`pnpm nx lint @lms/student-portal --configuration=fix`
...
`pnpm nx load-runtime-config @lms/student-portal`
